...
def get_flights(fly_from, fly_to, 
    ...
    Args:
        fly_from (String or double tuple): City from where you are leaving 
                                           (string - city name, 
                                           double tuple - lat,lon of city)
        ...
        Optional Args:
        return_from (String): Minimum return date 
                      (i.e, the earliest you would like to return to fly_from)
        return_to (String): Maximum return date 
                        (i.e., the latest you would like to return to fly_from)
        radius (String): Distance of airport from city. 
                         Default 50. Default is suggested.

    Return:
        final_data - List of dictionaries, each entry contains particular 
                     flight data
        Each entry of the form:
            {'price':price of the flight
             'Itinerary' - Holds all the legs of the flight and associated 
                           information
    ...
    fly_from = str(round(out_lat,2)) + '-' + str(round(out_lon,2)) + '-' + \
                                                    str(radius) + radius_format
    fly_to = str(round(in_lat,2)) + '-' + str(round(in_lon,2)) + '-' + \
                                                    str(radius) + radius_format

    url = ('https://api.skypicker.com/flights?fly_from={}&fly_to={}&'
           'date_from={}&date_to={}&curr={}&sort={}&partner=picky&v=3').format(\
          fly_from, fly_to, date_from, date_to, 'USD', 'price')
    print(url)
    #Case for roundtrip flights
    if roundtrip:
        assert return_from != None and return_to != None, \
    ...
    for i in range(0,len(final_data)):
        final_data[i] = {'price':filt_flight_data[i]['price'], 
                            ...
                            filt_flight_data[i]['fly_duration'].split('h')[0]),
                            "link":filt_flight_data[i]['deep_link'],
                            'start_dest':(filt_flight_data[i]['cityFrom'], \
                                            filt_flight_data[i]['flyFrom'], \
                                   filt_flight_data[i]['countryFrom']['name']),
                            'end_dest':(filt_flight_data[i]['cityTo'], \
                                            filt_flight_data[i]['flyTo'],\
                                      filt_flight_data[i]['countryTo']['name'])
                            }
