...
def get_hotels(location_id, 
    ...
    Args(Required)
        location_id (Str): TripAdvisor location_id obtained in get_location 
                           call
        num_adults (int): Number of adults needing accomodations
        rooms (int) : Number of rooms booked
    
    Args(Optional)
        check_in (Str): Check in date. Optional but highly suggested for 
                        relevance. Format YYYY-MM-DD 
        nights (int): Number of nights to stay.
        sort_by (Str): What to sort the query by 
        minprice (float): Minimum amount (/night) to be spent in given 
                          currency. Default Value 0 
        budget (float): Maximum amount (/night) that one would like to spend. 
                        Default Value 10000
    ...
    try:
        ...
        headers = {
            ...
            'x-rapidapi-key': ("21a93f3e0emsh34914a994184bdep12923cjsnc90566e5"
                               "ca81")
        }
        ...
        resp_data_price_sort = sorted(resp_data, key = \
            lambda x: (int(re.sub(',','',x['price'].split()[0].strip(',$,'))) \
                + int(re.sub(',','',x['price'].split()[2].strip(',$,')))/2))
